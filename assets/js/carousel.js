export class Carousel {
  constructor(options = {}) {
    try {
      const defaultOptions = {
        containerIdName: '#carousel', slidesClassName: '.slide__item', isAutoplayCarousel: true, autoPlayInterval: 10000, isControlButtonsEnabled: true, isControlPlayPauseEnabled: true, isIndicatorsEnabled: true, isPauseAfterAction: false, isAnimateButtonsHandler: true, isAnimateButtonAutoplay: false, isPauseWhenMouseFocus: false, userIcons: {}, userStyles: {},
      };
  ...
  _initProps() {
    ...
    const defaultIcons = {
      iconPause: '<i class="fas fa-pause" id="pause-btn"></i>', iconPlay: '<i class="fas fa-play" id="play-btn"></i>', iconPrev: '<i class="fas fa-angle-left" id="prev-btn"></i>', iconNext: '<i class="fas fa-angle-right" id="next-btn"></i>', iconDownload: '<i class="fa-solid fa-spinner"></i>',
    }
  ...
  _initControlItem() {
    ...
    this._createWrapper('control', 'control-container').innerHTML = this.isControlPlayPauseEnabled ? PREV + (!this.isAutoplayCarousel ? PLAY : PAUSE) + NEXT : PREV + NEXT
  }

  _initIndicatorItem() {
    const indicatorContainer = this._createWrapper('indicator', 'indicator-container')
  ...
  _initElements() {
    if (this.isControlButtonsEnabled) {
      if (this.isControlPlayPauseEnabled) { this.pauseBtn = this.container.querySelector('#pause-btn') }
  ...
  _initListeners() {
    ...
    if (this.isControlButtonsEnabled || this.isIndicatorsEnabled) this._initKeyListeners()
  }
  ...
  _setAutoPlayInterval() {
    this.timerId = setInterval(() => this._autoPlaySlides(), this.autoPlayInterval)
  }
  ...
  _toggleSlideClass() {
    ...
    if (this.isIndicatorsEnabled) { this.indicatorItems[this.currentSlide].classList.toggle('indicator__item--active') }
  }
  ...
  pausePlayHandler() {
    if (this.isControlButtonsEnabled && this.isControlPlayPauseEnabled) this.pausePlay()
  }
